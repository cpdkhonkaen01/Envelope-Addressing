...
const LAYOUT_SETTING_KEYS = [
  "garudaPlacement", "garudaSizeMm", "senderTopMm", "senderLeftMm", "senderTextOffsetMm", "senderFontPt",
  "recipientFontPt", "recipientTopPercent", "recipientLeftPercent", "recipientLineHeight",
  "postagePermitTopMm", "postagePermitRightMm", "postagePermitFontPt",
];
...
function collectManifestTracking() {
  ...
  elements.manifestRows?.querySelectorAll("tr[data-job-key]").forEach((row) => {
    const registered = row.querySelector('[name^="registered-"]')?.value || "";
    const ems = row.querySelector('[name^="ems-"]')?.value || "";
...
function saveCurrentPrintJobDraft(changes = {}, options = {}) {
  ...
  persistPrintHistory();
  if (elements.historySaveStatus) elements.historySaveStatus.textContent = "บันทึกอัตโนมัติแล้วเมื่อสักครู่";
...
function renderPrintHistory() {
  ...
  if (!state.printJobs.length) {
    elements.printHistoryList.innerHTML = '<div class="history-empty">ยังไม่มีประวัติ เมื่อเลือกรายชื่อ ระบบจะเริ่มบันทึกชุดงานให้อัตโนมัติ</div>';
    return;
  }
  elements.printHistoryList.innerHTML = state.printJobs.map((job) => {
    const status = printJobStatus(job);
    const recipientCount = Array.isArray(job.recipientIds) ? job.recipientIds.length : 0;
    const envelopeCount = Object.values(job.copiesById || {}).reduce((sum, value) => sum + Number(value || 0), 0);
    const names = (job.recipients || []).slice(0, 2).map((item) => item.name).filter(Boolean).join(" · ");
    return `<article class="history-item ${job.id === state.currentPrintJobId ? "current" : ""}">
      <div>
        <h3>ชุดงานวันที่ ${escapeHtml(formatPrintJobDate(job.createdAt))}</h3>
        <div class="history-item-meta"><span class="history-status ${status.className}">${status.label}</span><span>${recipientCount} รายชื่อ</span><span>${envelopeCount} ซอง</span><span>${escapeHtml(job.paperSize || "DL")}</span></div>
        ${names ? `<div class="manifest-recipient-sub">${escapeHtml(names)}${recipientCount > 2 ? " …" : ""}</div>` : ""}
      </div>
...
function formatTrackingCode(value = "", defaultPrefix = "") {
  ...
  if (!digits) return "";
  return `${String(defaultPrefix || "").toUpperCase()} ${digits} TH`;
}
...
function limitTrackingInput(event) {
  const cleaned = normalizeTrackingDigits(event.target.value);
...
function persistSettings() {
  rememberCurrentPaperLayout();
  const keys = [
    "sender", "senderAddress", "documentNumber", "paperSize", "paperLayouts", "showRecipientDepartment", "showRecipientAddress", "showSender", "showGaruda", "showPostagePermit",
    "garudaPlacement", "garudaSizeMm", "garudaStandardVersion", "senderTopMm", "senderLeftMm", "senderTextOffsetMm", "senderFontPt", "recipientFontPt",
    "recipientTopPercent", "recipientLeftPercent", "recipientLineHeight",
    "postagePermitText", "postagePermitTopMm", "postagePermitRightMm", "postagePermitFontPt",
  ];
...
function updateAdminAuthUi() {
  ...
  if (elements.historySyncHint) elements.historySyncHint.textContent = signedIn
    ? "เข้าสู่ระบบแล้ว ประวัติจะสำรองและเปิดต่อจากเครื่องอื่นได้"
    : "เข้าสู่ระบบผู้ดูแลเพื่อเปิดประวัติต่อจากเครื่องอื่นได้";
}

function openAdminAuthDialog(message = "") {
  elements.adminAuthEmail.value = state.settings.supabaseAdminEmail;
...
async function handleAdminAuthSubmit(event) {
  ...
  const button = $("#sendAdminMagicLink");
  button.disabled = true;
  button.textContent = "กำลังส่ง…";
  elements.adminAuthMessage.textContent = "กำลังส่งลิงก์เข้าสู่ระบบ…";
  try {
    const redirectUrl = state.settings.supabaseRedirectUrl || `${location.origin}${location.pathname}`;
    const { error } = await requireSupabase().auth.signInWithOtp({
      email: state.settings.supabaseAdminEmail,
      options: { emailRedirectTo: redirectUrl },
    });
    if (error) throw error;
    elements.adminAuthMessage.textContent = `ส่งลิงก์แล้ว กรุณาเปิดอีเมล ${state.settings.supabaseAdminEmail}`;
    elements.adminAuthMessage.className = "form-message success";
  } catch (error) {
    elements.adminAuthMessage.textContent = `ส่งลิงก์ไม่สำเร็จ: ${error.message}`;
    elements.adminAuthMessage.className = "form-message error";
  } finally {
    button.disabled = false;
    button.textContent = "ส่งลิงก์เข้าสู่ระบบ";
  }
}
...
async function initializeAdminAuth() {
  ...
  supabaseClient.auth.onAuthStateChange((_event, session) => {
    ...
    if (session && isAdminSignedIn()) {
      ...
      setNotice("เข้าสู่ระบบผู้ดูแลแล้ว");
      loadPrintJobsFromSupabase().then(renderPrintHistory).catch(console.warn);
    }
  });
}
...
function queuePrintJobCloudSave(job) {
  if (!isAdminSignedIn() || !job) return;
  clearTimeout(printJobCloudTimer);
  printJobCloudTimer = setTimeout(async () => {
    try {
      await savePrintJobToSupabase(job);
      if (elements.historySaveStatus) elements.historySaveStatus.textContent = "สำรองไว้ใน Supabase แล้ว";
    } catch (error) {
...
async function syncPrintHistory() {
  ...
  try {
    ...
    await loadPrintJobsFromSupabase();
    renderPrintHistory();
    elements.historySaveStatus.textContent = "สำรองไว้ใน Supabase แล้ว";
    setNotice(`ซิงค์ประวัติ ${state.printJobs.length} ชุดกับ Supabase แล้ว`);
  } catch (error) {
    console.error(error);
    elements.historySaveStatus.textContent = `ซิงค์ไม่สำเร็จ: ${error.message}`;
  } finally {
    button.disabled = false;
    button.textContent = "ซิงค์ประวัติ";
  }
}
...
function renderRows() {
  ...
  elements.rows.innerHTML = rows.map((item, index) => {
    ...
    return `<tr class="${checked ? "selected-row" : ""}">
      ...
      <td><span class="category-badge ${categoryClass(item.category)}">${escapeHtml(item.category)}</span>${classification ? `<small class="recipient-classification">${escapeHtml(classification)}</small>` : ""}</td>
      <td><span class="department-text">${escapeHtml(item.department || (item.category === "บุคคล" ? "บุคคล" : "ไม่ระบุหน่วยงาน"))}</span></td>
      <td><span class="address-text">${escapeHtml(recipientFullAddress(item))}</span></td>
...
function renderSummary() {
  ...
  postagePermitPreview.style.width = `${50 * previewScale}px`;
  postagePermitPreview.style.height = `${25 * previewScale}px`;
  postagePermitPreview.style.padding = `${2 * previewScale}px`;
  ...
  const previewRecipients = selectedPreviewRecipients.length
  ...
  if (previewRecipient) {
    ...
    const previewAddressHtml = state.settings.showRecipientAddress
      ...
      : "";
    receiverPreview.innerHTML = `<small>เรียน</small><h1>${escapeHtml(previewRecipientName)}</h1>${previewRecipient.position && previewRecipientName !== previewRecipient.position ? `<p>${escapeHtml(previewRecipient.position)}</p>` : ""}${previewDepartmentHtml}${previewAddressHtml}`;
  } else {
    receiverPreview.innerHTML = "<small>เรียน</small><h1>เลือกผู้รับเพื่อดูตัวอย่าง</h1>";
  }
  ...
  const receiverGreeting = receiverPreview.querySelector("small");
...
function fillSenderForm() {
  const fields = [
    ...
    "garudaSizeMm", "senderTopMm", "senderLeftMm", "senderTextOffsetMm", "senderFontPt",
  ];
  fields.forEach((name) => {
...
function fillPostagePermitForm() {
  const fields = ["postagePermitText", "postagePermitTopMm", "postagePermitRightMm", "postagePermitFontPt"];
  fields.forEach((name) => {
...
function openManifestDialog() {
  ...
  elements.manifestRows.innerHTML = jobs.map(({ item, copyIndex }, index) => {
    ...
    const subText = item.department && item.department !== name ? item.department : recipientFullAddress(item);
    ...
    return `<tr data-recipient-id="${escapeHtml(item.id)}" data-job-key="${escapeHtml(jobKey)}">
      <td>${index + 1}</td>
      <td><div class="manifest-recipient-name">${escapeHtml(name)}</div>${subText ? `<div class="manifest-recipient-sub">${escapeHtml(subText)}</div>` : ""}</td>
      <td>${escapeHtml(item.postalCode || "")}</td>
      <td><input name="registered-${index}" class="tracking-input" type="text" inputmode="numeric" autocomplete="off" maxlength="9" pattern="[0-9๐-๙]{9}" title="กรอกตัวเลข 9 หลัก" placeholder="ตัวเลข 9 หลัก" value="${escapeHtml(savedTracking.registered || "")}" /></td>
      <td><input name="ems-${index}" class="tracking-input" type="text" inputmode="numeric" autocomplete="off" maxlength="9" pattern="[0-9๐-๙]{9}" title="กรอกตัวเลข 9 หลัก" placeholder="ตัวเลข 9 หลัก" value="${escapeHtml(savedTracking.ems || "")}" /></td>
    </tr>`;
  }).join("");
  elements.manifestRows.querySelectorAll(".tracking-input").forEach((input) => {
    input.addEventListener("input", limitTrackingInput);
    input.addEventListener("input", () => saveCurrentPrintJobDraft());
  });
...
function printMailingManifest(event) {
  ...
  const invalidTrackingInput = [...elements.manifestRows.querySelectorAll(".tracking-input")]
    .find((input) => input.value.trim() && normalizeTrackingDigits(input.value).length !== 9);
  if (invalidTrackingInput) {
    invalidTrackingInput.setCustomValidity("กรุณากรอกตัวเลข 9 หลัก");
  ...
  const rows = jobs.map(({ item }, index) => ({
    ...
    registered: formatTrackingCode(form.get(`registered-${index}`), "RJ"),
    ems: formatTrackingCode(form.get(`ems-${index}`), "EQ"),
  }));
  const pageSize = 30;
  const pages = [];
  ...
  const manifestPages = pages.map((pageRows, pageIndex) => {
    ...
    const bodyRows = allRows.map((row) => `<tr>
      ...
      <td class="recipient-cell"${manifestRecipientFontStyle(row.name)}>${escapeHtml(row.name)}</td>
...
function printEnvelopes() {
  ...
  const pages = jobs.map(({ item }) => {
    ...
    const recipientAddressHtml = state.settings.showRecipientAddress
      ...
      : "";
    return `<section class="envelope">${senderBlockHtml}${postagePermitBlockHtml}<div class="recipient"><small>เรียน</small><h1>${escapeHtml(envelopeRecipientName)}</h1>${item.position && envelopeRecipientName !== item.position ? `<p class="recipient-position">${escapeHtml(item.position)}</p>` : ""}${recipientDepartmentHtml}${recipientAddressHtml}</div></section>`;
  }).join("");
  const popup = window.open("", "_blank");
  if (!popup) {
    setNotice("เบราว์เซอร์บล็อกหน้าพิมพ์ กรุณาอนุญาต Pop-up แล้วลองอีกครั้ง");
  ...
  popup.opener = null;
  popup.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>พิมพ์จ่าหน้าซอง</title><style>@page{size:${width} ${height};margin:0}*{box-sizing:border-box}body{margin:0;color:#17223b;font-family:"Noto Sans Thai",Tahoma,sans-serif}.envelope{position:relative;width:${width};height:${height};page-break-after:always;overflow:hidden;background:#fff}.envelope:last-child{page-break-after:auto}.sender{position:absolute;top:${senderTopMm}mm;left:${senderLeftMm}mm;width:55%;font-size:${senderFontPt}pt;line-height:1.45;color:#111}.sender-content{min-width:0}.sender-address{margin-top:.7mm}.document-number{margin-top:1.2mm;font-weight:700}.garuda{display:block;width:auto;height:${garudaSizeMm}mm;object-fit:contain;object-position:left top}.garuda-hidden{visibility:hidden}.sender.garuda-left{display:flex;align-items:flex-start;gap:3mm}.garuda-left .garuda{flex:0 0 auto}.garuda-left .sender-content{padding-top:${senderTextOffsetMm}mm}.garuda-above .garuda{margin:0 0 2mm 0}.postage-permit{position:absolute;top:${postagePermitTopMm}mm;right:${postagePermitRightMm}mm;display:flex;width:50mm;height:25mm;align-items:center;justify-content:center;overflow:hidden;padding:2mm;border:.35mm solid #111;color:#111;font-size:${postagePermitFontPt}pt;font-weight:700;line-height:1.45;text-align:center}.recipient{position:absolute;left:${recipientLeftPercent}%;top:${recipientTopPercent}%;width:calc(100% - ${recipientLeftPercent}% - 8mm);font-size:${recipientFontPt}pt;line-height:${recipientLineHeight}}.recipient small{color:#667085;font-size:${recipientFontPt}pt}.recipient h1{margin:1mm 0 0;font-size:${recipientFontPt}pt;white-space:nowrap}.recipient-position{white-space:nowrap}.recipient p{margin:.4mm 0}.organization{font-weight:700}@media screen{body{background:#eef2f7;padding:18px}.envelope{margin:0 auto 18px;box-shadow:0 12px 36px rgba(15,23,42,.14)}}</style></head><body>${pages}<script>addEventListener('load',()=>setTimeout(()=>print(),250));<\/script></body></html>`);
  popup.document.close();
...
$("#syncPrintHistory").addEventListener("click", syncPrintHistory);
